...
write_Log_args oprot record = do
  ...
  case f_Log_args_logBatch record of {Nothing -> return (); Just _v -> do
    ...
    (let f = Vector.mapM_ (\_viter57 -> write_LogMessage oprot _viter57) in do {writeListBegin oprot (T_STRUCT,fromIntegral $ Vector.length _v); f _v;writeListEnd oprot})
    writeFieldEnd oprot}
...
read_Log_args_fields iprot record = do
  ...
  if _t59 == T_STOP then return record else
    case _id60 of 
      1 -> if _t59 == T_LIST then do
        s <- (let f n = Vector.replicateM (fromIntegral n) ((read_LogMessage iprot)) in do {(_etype64,_size61) <- readListBegin iprot; f _size61})
